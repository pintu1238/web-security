...
const paths = {
  ...
  users:'<circle cx="9" cy="8" r="3"/><path d="M3 20v-2a6 6 0 0 1 12 0v2M16 5a3 3 0 0 1 0 6m1 3a6 6 0 0 1 4 4v2"/>',
...
const pages = [
  ...
  {id:'user',title:'User panel',icon:'users'},
];
const templates = [
...
function featureInteractive(feature){return state.projectConfig?.features?.[feature]?.interactive!==false;}
...
function partBOverviewNotice(){
  if(!state.projectConfig||state.projectConfig.stage!=='part_a')return '';
  return `<div class="notice warning stage-notice" role="status">${icon('lock')}<span><strong>Part A presentation mode.</strong> Security scanning, legal checklist actions and role-based panels are shown as frontend previews. Their Part B workflows remain protected until implementation is complete.</span></div>`;
}
...
async function api(path, options={}) {
  ...
  const timeout = setTimeout(()=>controller.abort(),path==='/search'?375000:60000);
  try {
...
async function renderPage(){
  ...
  try {
    ...
    if(version!==state.renderVersion)return;
    const renderers={dashboard:dashboardView,scanner:scannerView,compliance:complianceView,assistant:assistantView,documents:documentsView,knowledge:knowledgeView,admin:adminView,user:userView,settings:settingsView};
...
function dashboardView(){
  ...
  return heading('Overview','Here’s where your business stands. Let’s make it a little stronger.',
    `<a class="btn btn-secondary" href="/api/export">${icon('download')}<span class="optional-label">Export </span>report</a>${button('Run assessment',stageLocked('security_scanner')?'stage-locked':'navigate','scan','primary',`data-page="scanner" ${stageAttrs('security_scanner')}`)}`,
    'OVERVIEW')+
    partBOverviewNotice()+
    ...
    <div class="dashboard-columns"><div class="column-stack">
      ...
      <section class="card"><div class="card-header"><div><h2>Your next steps <span class="pill" style="margin-left:6px">${pending.length}</span></h2><p>Small actions. Meaningful progress.</p></div>${link('View all','compliance')}</div><div class="task-list">${pending.length?pending.slice(0,4).map(taskRow).join(''):empty('You’re all caught up','Add a task to plan your next improvement.',button('Add a task','add-task','plus','primary',stageAttrs('legal_compliance')))}</div><div class="card-foot"><span>Starter recommendations for your business</span>${button('Add task','add-task','plus','secondary',stageAttrs('legal_compliance'))}</div></section>
...
function complianceView(){
  ...
  return heading('A little progress, every day.','Keep your business essentials organised in one practical checklist.',button('Add task','add-task','plus','primary',stageAttrs('legal_compliance')),'LEGAL COMPLIANCE')+
  stageNotice('legal_compliance')+
...
function visibleConversations(){
  if(state.activeConversationIds===null)return state.conversations;
  const visible=new Set(state.activeConversationIds.map(String));
  return state.conversations.filter(conversation=>visible.has(String(conversation.id)));
}
function chatLanguageLabel(){return state.chatLanguage||'Auto';}
function chatDocumentLabel(){return state.chatDocument?state.knowledge.find(document=>document.id===state.chatDocument)?.title||'Selected document':'Auto · general chat + library';}
function assistantView(){return heading('A clearer way to find answers.','Ask everyday questions, get help with NitiShield, or explore your PDFs.','','LEGAL ASSISTANT')+
  `<div class="chat-layout"><section class="card chat-panel"><div class="chat-header"><div class="chat-heading"><span class="chat-avatar">${icon('sparkles')}</span><div><strong>NitiShield assistant</strong><small>Everyday answers · PDF explanations</small></div></div><details class="chat-menu"><summary class="icon-button chat-menu-trigger" aria-label="Chat options" title="Chat options">${icon('more-vertical')}</summary><div class="chat-menu-panel" role="menu"><button type="button" class="chat-menu-item" data-action="new-chat" ${state.busyChat?'disabled':''}>${icon('plus')}<span><strong>New chat</strong><small>Start a blank conversation</small></span></button><button type="button" class="chat-menu-item" data-action="answer-from" ${state.busyChat?'disabled':''}>${icon('document')}<span><strong>Answer from</strong><small>${esc(chatDocumentLabel())}</small></span></button><button type="button" class="chat-menu-item" data-action="recent-chats" ${state.busyChat?'disabled':''}>${icon('history')}<span><strong>Recent chats</strong><small>View previous chat history</small></span></button><button type="button" class="chat-menu-item" data-action="chat-language" ${state.busyChat?'disabled':''}>${icon('globe')}<span><strong>Chat language</strong><small>${esc(chatLanguageLabel())}</small></span></button><div class="chat-menu-divider"></div><button type="button" class="chat-menu-item danger" data-action="delete-selected-chat" ${state.busyChat||state.selectedConversationId===null?'disabled':''}>${icon('trash')}<span><strong>Delete selected chat</strong><small>${state.selectedConversationId===null?'Select a recent chat first':'Remove this chat only'}</small></span></button></div></details></div>${chatScope()}<div class="chat-messages" id="chat-messages" aria-live="polite">${visibleConversations().length?visibleConversations().map(conversationMarkup).join(''):chatWelcome()}</div><form class="chat-form" id="chat-form"><div class="chat-input-wrap"><textarea id="chat-question" name="question" rows="2" required maxlength="2000" placeholder="Ask anything, or choose a PDF to discuss..." aria-label="Your question" ${state.busyChat?'disabled':''}></textarea><button type="submit" aria-label="Send question" ${state.busyChat?'disabled':''}>${icon('send')}</button></div><p class="chat-disclaimer">AI can make mistakes. Check the cited pages for important legal decisions.</p></form></section><aside class="chat-sidebar"><div class="card"><h3>Ask naturally</h3><p>Everyday questions work without a PDF. Choose a document when you want an answer based on that PDF, with page references you can check. Follow up with “explain that simply.”</p><div style="margin-top:16px">${link('Browse knowledge base','knowledge')}</div></div><div class="card"><h3>English or नेपाली</h3><p>Auto follows your question’s language. For a Nepali PDF, choose नेपाली or ask in Nepali. PDF translation into a different language needs a separately configured model. Scanned images need OCR first.</p></div></aside></div>`;}
function chatScope(){return `<div class="chat-scope"><span class="small muted" role="status">${esc(state.assistantStatus?.message||'Checking local AI...')}</span></div>`;}
function adminView(){
  return heading('Admin panel','A central place to manage people, permissions and workspace activity.',button('Invite member','stage-locked','users','primary','disabled aria-disabled="true"'),'ADMINISTRATION')+
    stageNotice('admin_panel')+
    `<div class="stat-grid" aria-label="Administration metrics">${statCard('Team members','—','','users','','Part B workflow','Invites and roles','blue')}${statCard('Pending reviews','—','','checklist','blue','Part B workflow','Approvals to review','blue')}${statCard('Audit events','—','','history','amber','Part B workflow','Recorded actions','amber')}${statCard('Workspace role','Admin','','settings','purple','Frontend preview','Permissions planned','blue')}</div>
    <div class="dashboard-columns"><div class="column-stack"><section class="card"><div class="card-header"><div><h2>Team members</h2><p>Invite colleagues and assign the least access they need.</p></div><span class="pill">Part B</span></div><div class="card-body">${empty('Team management starts in Part B','The complete member table and invitation workflow are ready to be connected to authentication.')}</div></section><section class="card"><div class="card-header"><div><h2>Role permissions</h2><p>Review what each role can see and change.</p></div></div><div class="card-body"><div class="compliance-detail"><span>${icon('settings')} Admin</span><span class="muted small">Manage workspace</span></div><div class="compliance-detail"><span>${icon('user')} User</span><span class="muted small">Complete assigned work</span></div><div class="compliance-detail"><span>${icon('book')} Auditor</span><span class="muted small">Read-only reports</span></div></div></section></div><div class="column-stack right-column"><section class="card"><div class="card-header"><div><h2>Recent admin activity</h2><p>Important changes will appear here.</p></div>${icon('history')}</div><div class="card-body">${empty('No admin events yet','Part B will record invites, role changes, approvals and exports.')}</div></section><section class="insight-card"><div class="insight-label">${icon('lock')} LEAST PRIVILEGE</div><h3>Give each person only the access their work requires.</h3><p>Authentication and backend permission checks will protect these controls in Part B.</p></section></div></div>`;
}
function userView(){
  return heading('User panel','A focused view of the work assigned to you and the evidence you have submitted.',button('View my tasks','stage-locked','checklist','secondary','disabled aria-disabled="true"'),'MY WORK')+
    stageNotice('user_panel')+
    `<div class="stat-grid" aria-label="User work metrics">${statCard('Assigned tasks','—','','checklist','','Part B workflow','Tasks assigned to you','blue')}${statCard('Completed','—','','check-circle','blue','Part B workflow','Your progress','blue')}${statCard('Needs review','—','','clock','amber','Part B workflow','Submitted evidence','amber')}${statCard('Access level','Member','','user','purple','Frontend preview','Role-based access','blue')}</div>
    <div class="dashboard-columns"><div class="column-stack"><section class="card"><div class="card-header"><div><h2>My assigned work</h2><p>Tasks and deadlines from your workspace manager.</p></div><span class="pill">Part B</span></div><div class="card-body">${empty('Assigned work starts in Part B','Users will see only tasks and documents shared with their account.')}</div></section><section class="card"><div class="card-header"><div><h2>Evidence submissions</h2><p>Track items waiting for review.</p></div>${icon('upload')}</div><div class="card-body">${empty('No submissions yet','Part B will let users upload evidence and respond to review requests.')}</div></section></div><div class="column-stack right-column"><section class="card"><div class="card-header"><div><h2>Access summary</h2><p>Your permitted workspace areas.</p></div>${icon('lock')}</div><div class="card-body"><div class="compliance-detail"><span>${icon('checklist')} Assigned compliance tasks</span><span class="muted small">Planned</span></div><div class="compliance-detail"><span>${icon('book')} Shared knowledge sources</span><span class="muted small">Planned</span></div><div class="compliance-detail"><span>${icon('document')} Approved documents</span><span class="muted small">Planned</span></div></div></section><section class="insight-card"><div class="insight-label">${icon('sparkles')} WORK WITH CONFIDENCE</div><h3>Every assignment has a clear next step.</h3><p>Part B will connect this focused panel to real user accounts and review workflows.</p></section></div></div>`;
}
function recentChatsMarkup(){
  const conversations=[...state.conversations].reverse();
  if(!conversations.length)return empty('No recent chats','Ask a question and it will appear here.');
  return `<div class="recent-chat-list">${conversations.map(conversation=>`<div class="recent-chat-item ${String(state.selectedConversationId)===String(conversation.id)?'selected':''}"><button type="button" class="recent-chat-select" data-action="select-conversation" data-id="${esc(conversation.id)}"><span class="recent-chat-icon">${icon('history')}</span><span class="recent-chat-copy"><strong>${esc(conversation.question)}</strong><small>${esc(when(conversation.created_at)||'Saved chat')}</small></span></button><button type="button" class="icon-button recent-chat-delete" data-action="delete-conversation" data-id="${esc(conversation.id)}" aria-label="Delete chat: ${esc(conversation.question)}" title="Delete this chat">${icon('trash')}</button></div>`).join('')}</div>`;
}
...
function answerFromModal(){
  showModal('Answer from',`<form id="chat-document-form"><div class="field"><label for="chat-document-choice">Document</label><select id="chat-document-choice" name="document_id"><option value="">Auto · general chat + library</option>${state.knowledge.map(document=>`<option value="${esc(document.id)}" ${state.chatDocument===document.id?'selected':''}>${esc(document.title)}</option>`).join('')}</select><small>Choose a PDF when you want the answer grounded in that document.</small></div><div class="form-actions"><button type="button" class="btn btn-secondary" data-action="close-modal">Cancel</button><button type="submit" class="btn btn-primary">${icon('check')}Save source</button></div></form>`);
}
function deleteConversationModal(id){
  const conversation=state.conversations.find(item=>String(item.id)===String(id));
  if(!conversation)return;
  showModal('Delete this chat?',`<p>This removes only “${esc(conversation.question)}” from recent chats. Other conversations will stay saved.</p><div class="form-actions"><button type="button" class="btn btn-secondary" data-action="close-modal">Keep chat</button><button type="button" class="btn btn-danger" data-action="confirm-delete-conversation" data-id="${esc(id)}">${icon('trash')}Delete chat</button></div>`);
}
...
function renderAssistant(){if(state.page==='assistant'){main.innerHTML=`<div class="page-enter">${assistantView()}</div>`;scrollChat();}}
async function handleAction(el,event){
  ...
  switch(a){
    ...
    case 'delete-document':{const d=state.documents.find(document=>String(document.id)===el.dataset.id);if(!d)return;showModal('Delete this draft?',`<p>Delete “${esc(d.title)}” from your document library? This saved draft cannot be recovered.</p><div class="form-actions">${button('Keep draft','close-modal','close','secondary')}${button('Delete draft','confirm-delete-document','trash','danger',`data-id="${esc(d.id)}"`)}</div>`);break;}
    case 'confirm-delete-document':el.disabled=true;try{await api(`/documents/${encodeURIComponent(el.dataset.id)}`,{method:'DELETE'});modal.close();await refreshData();await renderPage();toast('Draft deleted.');}finally{el.disabled=false;}break;
    ...
    case 'new-chat':if(state.busyChat)return;state.activeConversationIds=[];state.selectedConversationId=null;state.chatDocument='';document.querySelector('.chat-menu')?.removeAttribute('open');renderAssistant();document.getElementById('chat-question')?.focus();break;
    case 'answer-from':if(state.busyChat)return;document.querySelector('.chat-menu')?.removeAttribute('open');answerFromModal();break;
    case 'recent-chats':if(state.busyChat)return;document.querySelector('.chat-menu')?.removeAttribute('open');recentChatsModal();break;
    case 'chat-language':if(state.busyChat)return;document.querySelector('.chat-menu')?.removeAttribute('open');chatLanguageModal();break;
    case 'select-conversation':{const conversation=state.conversations.find(item=>String(item.id)===String(el.dataset.id));if(!conversation)return;state.activeConversationIds=[conversation.id];state.selectedConversationId=conversation.id;modal.close();renderAssistant();break;}
    case 'delete-conversation':deleteConversationModal(el.dataset.id);break;
    case 'delete-selected-chat':if(!state.busyChat&&state.selectedConversationId!==null)deleteConversationModal(state.selectedConversationId);break;
    case 'confirm-delete-conversation':{
      ...
      try{
        ...
        state.conversations=state.conversations.filter(item=>String(item.id)!==String(id));
        if(state.activeConversationIds!==null)state.activeConversationIds=state.activeConversationIds.filter(item=>String(item)!==String(id));
        if(String(state.selectedConversationId)===String(id))state.selectedConversationId=null;
        modal.close();
        renderAssistant();
        toast('Selected chat deleted.');
...
async function handleForm(form){
  ...
  try{
    switch(form.id){
      ...
      case 'chat-language-form':state.chatLanguage=values.language||'';modal.close();renderAssistant();toast(`Chat language set to ${chatLanguageLabel()}.`);break;
      case 'chat-document-form':state.chatDocument=values.document_id||'';modal.close();renderAssistant();toast(`Answer source set to ${chatDocumentLabel()}.`);break;
...
async function sendQuestion(question,form){
  ...
  const startedConversationIds=state.activeConversationIds;
  const input=form.elements.question;input.disabled=true;
  const messages=document.getElementById('chat-messages');
  if(!visibleConversations().length)messages.innerHTML='';
  ...
  try{
    ...
    if(historyVersion===state.historyVersion){
      state.conversations=conversations;
      if(startedConversationIds!==null)state.activeConversationIds=[...startedConversationIds,answer.id].filter((id,index,array)=>array.indexOf(id)===index);
    }
    await refreshData();
  }
